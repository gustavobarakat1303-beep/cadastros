...
const REGRAS = [
  ...
  'A 2ª feijoada cortesia é de igual ou menor valor.',
...
export default function FeijoadaInvite() {
  ...
  if (notFound || !invite) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center px-6 text-center">
        ...
        <Link to="/feijoada" className="mt-4 text-sm underline">
          Gerar meu convite
        </Link>
      </div>
    )
  ...
  const qrUrl = `${window.location.origin}/feijoada/validar?code=${invite.code}`
    ...
    <div className="min-h-screen flex flex-col items-center px-6 py-12">
      <div className="w-full max-w-md flex flex-col items-center">
        ...
        <h1 className="mt-2 text-center text-2xl font-bold leading-snug">
          Seu convite da Feijoada está liberado! 💛
        </h1>
        ...
        <p className="mt-2 self-start text-gray-300">
          Te esperamos no Nômade para uma feijoada especial.
        </p>
        ...
        <div className="mt-6 w-full">
          <p className="text-sm font-semibold text-gray-400">Benefício</p>
          <p className="mt-1 text-gray-200">{invite.benefit}</p>
        </div>

        <div className="mt-6 w-full">
          <p className="text-sm font-semibold text-gray-400">Validade</p>
          <p className="mt-1 text-gray-200">Válido até {formatDate(invite.valid_until)}.</p>
        </div>
        ...
        <div className="mt-8 w-full rounded-2xl border border-gray-800 bg-gray-900/60 p-6 flex flex-col items-center">
          ...
          <p className="mt-1 text-2xl font-mono font-bold tracking-widest">{invite.code}</p>
          {active ? (
