/**
 * FeijoadaVoucher.jsx
 * Página do voucher/QR Code gerado após cadastro bem-sucedido.
 * Rota: /feijoada/convite/:code
 */
...
const NOMADE_WHATSAPP_BASE = 'https://wa.me/5511916547785?text=';
...
export default function FeijoadaVoucher() {
  ...
  const whatsappUrl = `${NOMADE_WHATSAPP_BASE}${encodeURIComponent(`Olá! Resgatei meu convite da Feijoada Cortesia. Meu código é: ${code}`)}`;
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (err || !data) {
          setError('Convite não encontrado. Verifique o código ou refaça o cadastro.');
          return;
        }
        setRegistration(data);
      } catch (e) {
        console.error('[FeijoadaVoucher]', e);
      ...
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-6">
          <p className="text-[#c8b89a] text-xs tracking-widest uppercase mb-2">
            {registration?.used ? 'Convite já utilizado' : 'Seu convite está pronto'}
          </p>
          <h1 className="text-[#f4efe8] text-3xl font-black uppercase tracking-tight leading-none mb-2">
            1 Feijoada<br />Cortesia
          </h1>
          <p className="text-[#c8b89a] text-sm tracking-wider mt-2">
            Olá, {registration?.name?.split(' ')[0]}!
          </p>
        ...
        <div className="border border-[#c8b89a]/20 p-6 flex flex-col items-center gap-5 mb-6">
          ...
          {registration?.used && (
            <p className="text-amber-400/70 text-xs text-center">
              Utilizado em {new Date(registration.used_at).toLocaleDateString('pt-BR')}
            </p>
          )}
        </div>
        ...
        <div className="text-center mt-8">
          <p className="text-[#c8b89a]/30 text-xs">
            Nômade · Av. Rebouças, 3400 · Pinheiros, São Paulo
          </p>
        </div>
